...
import chumpy
import numpy as np
#SMPL model rigid tranformation corresponding to the Formula 5
# pose:theta J:joint location 
def global_rigid_transformation(pose, J, kintree_table, xp):
    ...
    results[0] = with_zeros(xp.hstack((rodrigues(pose[0,:]), J[0,:].reshape((3,1)))))# root transform        
    # G'k()    
...
def verts_core(pose, v, J, weights, kintree_table, want_Jtr=False, xp=chumpy):
    A, A_global = global_rigid_transformation(pose, J, kintree_table, xp)  # 4 x 4 x 24
    T = A.dot(weights.T) # every vertex transformation 4 x 4 x n 
